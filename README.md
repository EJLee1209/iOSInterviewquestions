...
- Delegate란 무엇인지 설명하고, retain 되는지 안되는지 그 이유를 함께 설명하시오.
  ...
  > Delegate는 iOS 프레임워크에서 자주 사용하는 디자인 패턴으로, 한 객체가 다른 객체에게 특정 작업을 위임할 때 사용합니다. 서로 다른 객체가 서로를 참조하기 때문에 강하게 참조하는 경우 retain됩니다. 이 경우에 Retain Cycle이 발생할 수 있기 때문에 delegate를 약한 참조로 선언해 Retain Cycle 문제를 예방할 수 있습니다.
...
- struct와 class와 enum의 차이를 설명하시오.
  ...
  > struct와 enum은 값 타입이고, class는 참조 타입입니다. 값 타입은 값을 전달할 때, 인스턴스를 복사해서 전달하고, 참조 타입은 인스턴스의 참조를 전달합니다. 클래스는 상속이 가능하지만, 구조체와 열거형은 상속이 불가능합니다. 구조체와 클래스는 데이터를 용도에 맞게 묶어 표현하고자할 때 유용합니다. 캡슐화한 값을 참조하는 것보다 복사하는 것이 합당하면, 구조체를 사용하고 그렇지 않다면 클래스를 사용하는 것이 좋습니다.
  > 열거형은 연관된 항목들을 묶어서 표현할 수 있는 타입입니다. 따라서 제한된 선택지를 주고 싶을 때 유용합니다.

- class의 성능을 향상 시킬수 있는 방법들을 나열해보시오.

  > 상속이 필요없는 class를 final로 선언하고, 파일 내에서만 접근해도 되는 경우에 private로 선언함으로써 성능을 향상시킬 수 있습니다.
  > class에 final이 붙으면 상속 자체가 불가능해져서 런타임 시점에 오버라이딩 여부를 체크할 필요가 없기 때문에, 이런 체크 과정으로 인한 성능 저하를 방지할 수 있습니다. 또 다른 성능 향상 방법으로 파일 내에서만 접근해도 되는 경우에 private로 선언하는 방법이 있습니다. 컴파일러는 private가 붙은 프로퍼티에 대해서 오버라이드가 될 수 있는지 없는지를 판단해 오버라이드하는 곳이 없다면, final로 추론하게 됩니다.
...
- AnyObject에 대해 설명하시오.
  ...
  > AnyObject는 참조 타입을 표현하는 프로토콜입니다. 즉, 모든 클래스 타입의 공통된 슈퍼타입으로 사용됩니다.
...
- Struct 가 무엇이고 어떻게 사용하는지 설명하시오.
  ...
  > Struct는 데이터를 용도에 맞게 묶어 표현하고, 새로운 커스텀 데이터 타입을 만들 수 있도록 Swift에서 제공해주는 기능입니다. struct 키워드를 사용해서 새로운 구조체를 선언하고 그 안에 프로퍼티와 메서드들을 정의해서 사용합니다.
...
- ARC란 무엇인지 설명하시오.
  ...
  > iOS에서는 메모리를 자동으로 관리해주는 ARC기술을 사용합니다. ARC는 Automatic Reference Counting의 약자로, 참조 타입 인스턴스의 참조 횟수를 계산하고, 참조 횟수가 0이면, 더 이상 사용되지 않는 인스턴스로 판단해 메모리에서 해제합니다.
...
- Strong 과 Weak 참조 방식에 대해 설명하시오.
  > Strong은 강한 참조로, 참조하는 인스턴스의 Reference Count(참조 횟수)를 증가시켜 참조하는 동안 메모리에서 해제되지 못하도록 합니다. 반면에 Weak는 약한 참조로, 참조하는 인스턴스의 참조 횟수를 증가시키지 않고, 인스턴스가 메모리에서 해제되면 자동으로 nil이 할당됩니다.
- 순환 참조에 대하여 설명하시오.
  > 순환 참조는 서로 다른 인스턴스가 서로를 강하게 참조하면, 두 인스턴스의 참조 횟수가 0에 도달하지 못해 앱이 종료될 때까지 메모리에서 해제되지 못하는 상황을 순환 참조(Retain Cycle)라고합니다.
